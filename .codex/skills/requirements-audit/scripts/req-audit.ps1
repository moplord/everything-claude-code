...
foreach ($f in $reqFiles) {
  ...
  if ($rid) {
    ...
    if (!(Test-Path $acceptancePath)) {
      ...
      $accText = Get-Content -Raw -Encoding UTF8 $acceptancePath
      foreach ($ac in $acIds) {
  ...
  if (!(Test-Path $appendixPath)) {
    ...
    if ($axType -and ($axType.Trim() -eq "domain-model")) {
      ...
      if (($ax -notmatch '(?m)^\|\s*Field\s*\|\s*Meaning') -and ($ax -notmatch '(?m)^\|\s*\u5b57\u6bb5\s*\|\s*\u4e1a\u52a1\u542b\u4e49')) {
      ...
      if (($ax -notmatch '(?m)^\|\s*Name\s*\|\s*Entity A\s*\|\s*Entity B\s*\|') -and ($ax -notmatch '(?m)^\|\s*\u5173\u7cfb\u540d\s*\|')) {
      ...
      if (($ax -notmatch '(?m)^\|\s*Scope\s*\|\s*Entity\.?Field') -and ($ax -notmatch '(?m)^\|\s*Scope\s*\|\s*\u5b9e\u4f53\.?\u5b57\u6bb5')) {
      ...
      if ($ax -match '(?m)^\|\s*Field\s*\|\s*Meaning\s*\|\s*Type Candidates') {
      ...
      if ($ax -match '(?m)^\|\s*\u5b57\u6bb5\s*\|\s*\u4e1a\u52a1\u542b\u4e49\s*\|\s*\u7c7b\u578b\u5019\u9009') {
        Add-Issue -Severity "ERROR" -Path $appendixPath -Message "consumer-feature appendix appears to include a domain-model Field Dictionary table header. Domain model must live only in Type=domain-model."
      }
