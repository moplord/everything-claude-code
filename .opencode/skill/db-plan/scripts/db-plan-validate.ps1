...
foreach ($f in $domainReqs) {
  ...
  if (-not $rid) { continue }
  $matches = Get-ChildItem -Path $OutRoot -Recurse -File -Filter ($rid + ".db-plan.md") -ErrorAction SilentlyContinue
...
if ($missing.Count -gt 0) {
  Fail ("Missing db-plan outputs for: " + ($missing -join ", ") + ". Run db-plan-generate.ps1 first.")
}

Write-Host "PASS: db-plan outputs exist for all selected domain-model REQs."
exit 0
